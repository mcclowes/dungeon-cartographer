...
/**
 * Add subtle worn parchment edges around the canvas border
 */
export function addParchmentEdges(
  ...
  // Subtle darkening along edges with irregular worn look
  ctx.fillStyle = "rgba(90, 65, 35, 0.4)";

  // Top edge - gentle worn appearance
  ctx.beginPath();
  ctx.moveTo(0, 0);
  let x = 0;
  while (x < width) {
    const segmentWidth = 12 + rand() * 20;
    const wearDepth = maxEdge * (0.4 + rand() * 0.6);

    ctx.lineTo(x, wearDepth * (0.6 + rand() * 0.4));
    x += segmentWidth;
  }
  ctx.lineTo(width, 0);
  ctx.closePath();
  ...
  while (x < width) {
    ...
    ctx.lineTo(x, height - wearDepth * (0.6 + rand() * 0.4));
    x += segmentWidth;
  }
  ...
  let y = 0;
  while (y < height) {
    const segmentHeight = 12 + rand() * 20;
    const wearDepth = maxEdge * (0.4 + rand() * 0.6);

    ctx.lineTo(wearDepth * (0.6 + rand() * 0.4), y);
    y += segmentHeight;
  }
  ctx.lineTo(0, height);
  ctx.closePath();
  ...
  while (y < height) {
    ...
    ctx.lineTo(width - wearDepth * (0.6 + rand() * 0.4), y);
    y += segmentHeight;
  }
  ctx.lineTo(width, height);
  ctx.closePath();
  ...
  // Sparse age spots near edges
  const spotCount = Math.floor((width + height) / 200);
  for (let i = 0; i < spotCount; i++) {
    ...
    switch (edge) {
      case 0:
        ...
        break;
      case 1:
        sx = width - rand() * maxEdge * 2;
        sy = rand() * height;
        break;
      case 2:
        sx = rand() * width;
        sy = height - rand() * maxEdge * 2;
        break;
      default:
        sx = rand() * maxEdge * 2;
        sy = rand() * height;
    ...
    const spotSize = 3 + rand() * 8;
    ctx.beginPath();
    ctx.arc(sx, sy, spotSize, 0, Math.PI * 2);
    ctx.fillStyle = `rgba(70, 50, 30, ${0.05 + rand() * 0.08})`;
    ctx.fill();
  }
